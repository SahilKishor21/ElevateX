...
interface MetricsStore {
  ...
  clearAlerts: () => void;
  updateChartData: (
    type: keyof MetricsStore["chartData"],
    data: ChartDataPoint[]
  ) => void;
...
export const useMetricsStore = create<MetricsStore>((set, get) => ({
  ...
  updatePerformanceMetrics: (metrics) => {
    ...
    set((state) => {
      ...
      if (
        typeof updatedMetrics.averageWaitTime !== "number" ||
        isNaN(updatedMetrics.averageWaitTime)
      ) {
        updatedMetrics.averageWaitTime = 0;
      }
      if (
        typeof updatedMetrics.starvationCount !== "number" ||
        isNaN(updatedMetrics.starvationCount)
      ) {
  ...
  updateRealTimeMetrics: (metrics) => {
    ...
    set((state) => {
      ...
      if (
        typeof updatedMetrics.activeRequests !== "number" ||
        isNaN(updatedMetrics.activeRequests)
      ) {
        updatedMetrics.activeRequests = 0;
      }
      if (
        typeof updatedMetrics.starvationAlerts !== "number" ||
        isNaN(updatedMetrics.starvationAlerts)
      ) {
  ...
  updateAssignmentMetrics: (assignmentMetrics) => {
    ...
    debugMetrics(
      "MetricsStore - Updating Assignment Metrics",
      assignmentMetrics
    );
    ...
    set((state) => {
      const updatedMetrics = {
        ...
        ...assignmentMetrics,
        lobbyToUpperRequests:
          typeof assignmentMetrics.lobbyToUpperRequests === "number"
            ? assignmentMetrics.lobbyToUpperRequests
            : state.assignmentMetrics?.lobbyToUpperRequests || 0,
        upperToLobbyRequests:
          typeof assignmentMetrics.upperToLobbyRequests === "number"
            ? assignmentMetrics.upperToLobbyRequests
            : state.assignmentMetrics?.upperToLobbyRequests || 0,
        peakHourRequests:
          typeof assignmentMetrics.peakHourRequests === "number"
            ? assignmentMetrics.peakHourRequests
            : state.assignmentMetrics?.peakHourRequests || 0,
        starvationEvents:
          typeof assignmentMetrics.starvationEvents === "number"
            ? assignmentMetrics.starvationEvents
            : state.assignmentMetrics?.starvationEvents || 0,
        thirtySecondEscalations:
          typeof assignmentMetrics.thirtySecondEscalations === "number"
            ? assignmentMetrics.thirtySecondEscalations
            : state.assignmentMetrics?.thirtySecondEscalations || 0,
      };
  ...
  updateAssignmentCompliance: (assignmentCompliance) => {
    if (!assignmentCompliance || typeof assignmentCompliance !== "object") {
      console.warn(
        "Invalid assignment compliance received:",
        assignmentCompliance
      );
      return;
    }
    ...
    debugMetrics(
      "MetricsStore - Updating Assignment Compliance",
      assignmentCompliance
    );
    ...
    set((state) => {
      const updatedCompliance = {
        ...
        lobbyTrafficPercentage:
          typeof assignmentCompliance.lobbyTrafficPercentage === "number"
            ? assignmentCompliance.lobbyTrafficPercentage
            : state.assignmentCompliance?.lobbyTrafficPercentage || 0,
        peakHourRequests:
          typeof assignmentCompliance.peakHourRequests === "number"
            ? assignmentCompliance.peakHourRequests
            : state.assignmentCompliance?.peakHourRequests || 0,
        starvationEvents:
          typeof assignmentCompliance.starvationEvents === "number"
            ? assignmentCompliance.starvationEvents
            : state.assignmentCompliance?.starvationEvents || 0,
        thirtySecondEscalations:
          typeof assignmentCompliance.thirtySecondEscalations === "number"
            ? assignmentCompliance.thirtySecondEscalations
            : state.assignmentCompliance?.thirtySecondEscalations || 0,
        complianceScore:
          typeof assignmentCompliance.complianceScore === "number"
            ? assignmentCompliance.complianceScore
            : state.assignmentCompliance?.complianceScore || 100,
      };
...
export const selectAssignmentCompliance = (state: MetricsStore) => {
  ...
  return typeof compliance === "number" && !isNaN(compliance)
    ? compliance
    : 100;
};
